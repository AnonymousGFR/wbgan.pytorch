...
def train_func(data_loader, G, D, G_ema, ema, z_train, g_optimizer, d_optimizer, z_sample, train_dict, args, myargs):
  def train(**kwargs):
    for i, (imgs, _) in enumerate(tqdm.tqdm(data_loader)):
      ...
      div_gp = gan_losses.wgan_div_gradient_penalty(real_imgs=imgs, fake_imgs=f_imgs,
                                                    real_validity=r_logit, fake_validity=f_logit)
      d_loss = -wd + div_gp + torch.relu(wd - sinkhorn_d.item())
      summary_wd_sinkhornd['wd'] = wd.item()
      summary['div_gp'] = div_gp.item()
      ...
      if step % args.n_critic == 0:
        ...
        f_logit = D(f_imgs)
        f_logit_mean = f_logit.mean()
        g_loss = - f_logit_mean + sinkhorn_d
